...
def cnn_gru(input_dim, output_dim, dropout=0.2):
    # Input data type
    ...
    model = Sequential([
        ...
        BatchNormalization(),
        Dropout(dropout),
        GRU(512, activation='tanh', return_sequences=True, recurrent_activation='sigmoid', dropout=dropout),
        GRU(512, activation='tanh', return_sequences=False, recurrent_activation='sigmoid', dropout=dropout),
...
K.clear_session()
model3 = cnn_gru(input_dim, n_classes)
model3.summary()
...
plot_confusion_matrix(y_val.argmax(axis=1),pred3.argmax(axis=1), normalize=True, classes=classes, filename="model3_conf_mat")